...
在go语言中，抢占调度是通过采用协作式和抢占式混合调度实现的。当协程主动发起IO操作、调用runtime.Gosched()
...
在Linux内核5.1版本引入了新的异步IO框架io_uring，其真正意义上实现了异步IO操作。性能方面，由于采用共享内存的方式来传递参数，io_uring支持用户态与内核态之间的零拷贝数据传输。它通过RingBuffer的方式实现批量处理IO请求，使得调用系统调用的次数大幅减少，在sqpoll模式下甚至能做到无系统调用。对于IO性能要求极高的场景，参照DPDK/SPDK，io_uring也提供了iopoll模式，允许用户牺牲一定CPU来换取更高的IO吞吐量和更低的延迟；扩展性方面，io_uring统一了网络IO和本地IO的接口，支持更复杂的IO场景，比如文件读写、网络编程等。
